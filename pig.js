/*
54 (ルールを追加する)
55,2つのロールを連続して6回投げたときに、スコア全部を失う
56,HTMLに入力フィールドを追加して、プレイヤーが勝利スコアを
設定できるにゅりょくフィールドです
57,ゲームに二番目のダイスを追加すること
...
document.querySelector('.btn-roll').addEventListener('click', function() {
  if (gamePlaying) {
    ...
    if (dice === 6 && lastDice === 6) {
      //55-1,この条件はdice-6を二回出したらすべてのスコア失う
      //Player looses score
...
document.querySelector('.btn-hold').addEventListener('click', function() {
  if (gamePlaying) {
    ...
    */
    ...
    /* 56-2,↑↑↑
    テキストコンテンツやHTMLなどを使用する代わりに
    単にvalueプロパティを使用しました
    ↑これによりユーザーがこの入力フィールドに入力した内容が
    表示されます
    ...
    だけどconsoleはいらない
    未定義、ゼロ、null,空の文字列のすべての値がこのようにfalse
    に強制されます
    それ以外の物は真実になります
    */
